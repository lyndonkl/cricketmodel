...
The graph contains \textbf{6 node type categories} with \textbf{21 context nodes}:
...
\begin{tikzpicture}[
    ...
    \node[nodetype, fill=actorcolor!40] (actor) at (4, 4) {
        \textbf{ACTOR}\\
        striker, nonstriker,\\bowler, partnership\\(7 nodes)
    };
    ...
    \node[nodetype, fill=ballcolor!40, minimum width=4cm] (ball) at (2, 1.5) {
        \textbf{BALL}\\
        15 features + embeddings\\(N nodes, full innings!)
    };
...
\begin{tikzpicture}[
    ...
    \node[box, fill=purple!15] (temporal) at (4, 3) {
        ...
        ball $\leftrightarrow$ ball (same\_matchup)
    };
    ...
    \node[below=0.3cm of cross, font=\small] {16 edge types total (*precedes has edge features)};
\end{tikzpicture}
...
\begin{itemize}
    \item \textbf{Cross-domain edges use GATv2Conv} (not SAGEConv): This allows the model to attend more to recent/relevant balls when aggregating a player's performance
    \item \textbf{Temporal edges have edge features}: The \texttt{precedes} edges carry a temporal distance attribute (closer to 0 = more recent) enabling recency-weighted attention
    \item \textbf{Cross-domain edges respect Z2 symmetry}: Only balls actually faced by current striker connect to striker\_identity (not all balls)
\end{itemize}
...
\newpage
%==========================================
\section{Ball Node Features}
%==========================================

Each ball node contains \textbf{15 numeric features} plus player embeddings:
...
\midrule
\multirow{5}{*}{Basic}
& runs & Normalized by 6 \\
& is\_wicket & Binary indicator \\
& over & Normalized by 20 \\
& ball\_in\_over & Normalized by 6 \\
& is\_boundary & 4 or 6 scored \\
\midrule
\multirow{4}{*}{Extras}
& is\_wide & Wide ball indicator \\
& is\_noball & No-ball indicator \\
& is\_bye & Bye runs indicator \\
& is\_legbye & Leg bye indicator \\
\midrule
\multirow{6}{*}{Wicket Type}
& wicket\_bowled & Bowled dismissal \\
& wicket\_caught & Caught dismissal \\
& wicket\_lbw & LBW dismissal \\
& wicket\_run\_out & Run out dismissal \\
& wicket\_stumped & Stumped dismissal \\
& wicket\_other & Other dismissal types \\
\bottomrule
\end{tabular}
\caption{Ball node features (15 dimensions)}
\end{table}

\textbf{Why wicket types matter:}
\begin{itemize}
    \item \textbf{Bowled/LBW}: Indicates bowler skill, good line and length
    \item \textbf{Caught}: Suggests batsman aggression/risk-taking
    \item \textbf{Run out}: Partnership running decisions and risk assessment
    \item \textbf{Stumped}: Batsman error against spin bowling
\end{itemize}

Additionally, each ball has:
\begin{itemize}
    \item \textbf{Bowler ID}: 64-dim learned embedding of who bowled
    \item \textbf{Batsman ID}: 64-dim learned embedding of who faced
\end{itemize}

Total ball input: 15 + 64 + 64 = 143 dimensions $\to$ projected to hidden\_dim (128).
