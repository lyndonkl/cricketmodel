...
Node Name & Features & What it Represents \\
\midrule
\texttt{score\_state} & 5 features & Current score, wickets, balls, innings, \textbf{is\_womens\_cricket} \\
\texttt{chase\_state} & 7 features & Runs needed, RRR, is\_chase, RRR normalized, chase difficulty, balls remaining, wickets remaining (2nd innings) \\
\texttt{phase\_state} & 6 features & Powerplay/middle/death phase, over progress, \textbf{is\_first\_ball}, \textbf{is\_super\_over} \\
\texttt{time\_pressure} & 3 features & Balls remaining, urgency, is\_final\_over \\
\texttt{wicket\_buffer} & 2 features & Wickets in hand, is\_tail indicator \\
\bottomrule
\end{tabular}
...
Node Name & Features & What it Represents \\
\midrule
\texttt{striker\_identity} & Hierarchical embedding & WHO is on strike (player$\to$team$\to$role fallback for unknown players) \\
\texttt{striker\_state} & 8 features & HOW they're batting: runs, balls, SR, dot\%, boundaries, set indicator, \textbf{is\_debut\_ball}, \textbf{balls\_since\_on\_strike} \\
\texttt{nonstriker\_identity} & Hierarchical embedding & WHO is at the non-striker's end (with cold-start fallback) \\
\texttt{nonstriker\_state} & 8 features & Z2 symmetric with striker: runs, balls, SR, dot\%, boundaries, set, is\_debut\_ball, \textbf{balls\_since\_as\_nonstriker} \\
\texttt{bowler\_identity} & Hierarchical embedding & WHO is bowling (with cold-start fallback) \\
\texttt{bowler\_state} & 8 features & HOW they're bowling: overs, runs, wickets, economy, dot\%, \textbf{is\_pace}, \textbf{is\_spin} \\
...
\textbf{Z2 Symmetry:} Striker and non-striker states are now symmetric (both 8 features). The \texttt{balls\_since\_on\_strike} and \texttt{balls\_since\_as\_nonstriker} features capture the ``cold restart'' effect symmetrically: a batsman who hasn't faced for 12+ balls may be less ``in rhythm''.
...
Node Name & Features & What it Represents \\
\midrule
\texttt{batting\_momentum} & 1 feature & Recent scoring rate vs expectation \\
\texttt{bowling\_momentum} & 1 feature & Recent economy vs expectation \\
\texttt{pressure\_index} & 1 feature & Combined RRR, wicket, and time pressure \\
\texttt{dot\_pressure} & 5 features & Consecutive dots, balls since boundary, \textbf{balls\_since\_wicket}, \textbf{pressure\_accumulated}, \textbf{pressure\_trend} \\
\bottomrule
\end{tabular}
...
\midrule
18 numeric features & runs, is\_wicket, over, ball\_in\_over, is\_boundary, extras (wide, noball, bye, legbye), wicket types (bowled, caught, lbw, run\_out, stumped, other), striker\_run\_out, nonstriker\_run\_out, \textbf{bowling\_end} \\
64-dim bowler ID & Embedding of who bowled this ball \\
64-dim batsman ID & Embedding of who faced this ball \\
\bottomrule
\end{tabular}
\caption{Ball nodes: one per historical delivery (with run-out attribution and bowling end)}
\end{table}

\textbf{Run-out attribution:} The striker\_run\_out and nonstriker\_run\_out features disambiguate WHO was run out, enabling better risk assessment.

\textbf{Bowling end:} The bowling\_end feature (0 or 1) captures which end of the pitch the bowler is bowling from, as this can affect pitch behavior (footmarks, wear patterns).

\textbf{Total per ball:} 18 + 64 + 64 = 146 features $\to$ projected to 128-dim.
...
\begin{itemize}
    ...
    \item \textbf{Spell window (7-24 balls):} 4-over spell momentum, bowler fatigue patterns
    \item \textbf{Historical (25+ balls):} Phase patterns, earlier wickets' impact
\end{itemize}
...
\texttt{medium\_precedes} & Balls 7-24 apart & TransformerConv (spell-length window) \\
\texttt{distant\_precedes} & Balls 25+ apart (sparse) & SAGEConv (simple mean, sparse connections) \\
\bottomrule
\end{tabular}
...
\begin{tikzpicture}[
    ...
    \node[above=0.5cm of b6, align=center, font=\small, blue!60] {\texttt{medium\_precedes}\\(7-24 balls, spell window)};
\end{tikzpicture}
\caption{Multi-scale temporal edges (distant not shown for clarity)}
\end{figure}

\subsubsection{Same-Bowler and Same-Batsman Edges (with Temporal Decay)}

These edges connect balls by the same player, with \textbf{temporal decay edge attributes}:
...
\textbf{Temporal decay edge attributes:}
\begin{itemize}
    \item \texttt{same\_bowler}: $1.0 - (\text{ball\_distance} / 24.0)$ (4-over spell window)
    \item \texttt{same\_batsman}: $1.0 - (\text{ball\_distance} / 60.0)$ (10-over innings window)
\end{itemize}

Recent balls matter more than distant ones. The model uses \texttt{TransformerConv} with \texttt{edge\_dim=1} to learn attention patterns that respect this decay.

\textbf{Why this matters:} The model can ask ``How has Bumrah been bowling \textit{recently}?'' by looking at his deliveries with recency weighting, without needing to attend equally to all balls.

\subsubsection{Same-Over Edges (CAUSAL)}

\texttt{same\_over} connects balls within the same over. This captures within-over context:
\begin{itemize}
    \item Same bowler rhythm and plan for the over
    \item Field placement context
    \item Batsman reading the bowler
\end{itemize}

\textbf{Critical:} Like same\_matchup, same\_over edges are \textbf{CAUSAL (older $\to$ newer only)} to prevent train-test distribution shift.

\subsubsection{Same-Matchup Edges (CAUSAL)}

\texttt{same\_matchup} connects balls with the same bowler-batsman combination:

\textbf{Critical:} Both same\_over and same\_matchup edges are \textbf{UNIDIRECTIONAL (older $\to$ newer)}. This prevents ``future information leakage'':
...
Temporal & 7 & Ball history (multi-scale + actor grouping + \textbf{same\_over}) \\
...
\bottomrule
\end{tabular}
\caption{20 edge types organized by purpose (now includes same\_over edges)}
\end{table}
...
\begin{tikzpicture}[
    ...
    \node[stage, fill=blue!10] (s1) at (0, 6) {
        ...
        \textbf{Hierarchical player embeddings} (player$\to$team$\to$role fallback)\\
    ...
    \node[stage, fill=green!10] (s2) at (0, 2.5) {
        \textbf{Stage 2: Message Passing (with Enhanced FiLM Modulation)}\\[0.2cm]
        HeteroConv layers ($\times$3) with \textbf{FiLM conditioning}\\
        FiLM($\gamma$, $\beta$) from: phase\_state + chase\_state + wicket\_buffer (15 features)
    };

    % Stage 3
    \node[stage, fill=red!10] (s3) at (0, -1) {
        \textbf{Stage 3: Innings-Conditional Prediction}\\[0.2cm]
        Matchup MLP: concat(striker, bowler) $\to$ 128-dim\\
        Query projection: query $\to$ 128-dim\\
        \textbf{1st innings head} OR \textbf{2nd innings head} (with chase state injection)
    };
    ...
    % Input/Output
    \node[above=0.3cm of s1] {Input: HeteroData (nodes + edges + is\_chase flag)};
    \node[below=0.3cm of s3] {Output: [P(dot), P(1), P(2), P(3), P(4), P(6), P(wicket)]};
\end{tikzpicture}
\caption{Three-stage model pipeline with FiLM modulation and innings-conditional heads}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Non-striker gated modulation (P1.1):} For running outcomes (singles, twos, threes, run-outs), the non-striker's attributes matter. A learnable gate modulates the matchup: $\text{matchup} = \text{base\_matchup} \times (1 + 0.1 \times \sigma(\text{MLP}(\text{nonstriker})))$
    \item \textbf{Query aggregation:} Global context (venue, phase, momentum) aggregated by the query node
\end{enumerate}

This respects the geometric structure: prediction is at the matchup level, influenced by graph-level context and the non-striker's running ability.
...
\textbf{same\_bowler/batsman} & \textbf{TransformerConv} & \textbf{Temporal decay edge features (4/10-over)} \\
same\_matchup (causal) & GATv2Conv & Historical matchup outcomes \\
\textbf{same\_over (CAUSAL)} & \textbf{TransformerConv} & \textbf{Ball-in-over position edge features} \\
Cross-domain (faced/bowled/partnered) & GATv2Conv & Attention-weighted recency \\
Dynamics (informs) & \textbf{GATv2Conv} & \textbf{Attention for important balls} \\
Dynamics (drives) & GATv2Conv & Weight momentum vs pressure \\
Query (attends) & GATv2Conv & Learn what matters for prediction \\
\bottomrule
\end{tabular}
\caption{Edge-type-specific convolution operators (updated with temporal decay and same\_over)}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{N ball nodes} capture full innings history (18 features including run-out attribution, bowling end)
    \item \textbf{20 edge types} encode cricket-specific relationships (now includes same\_over)
    \item \textbf{Multi-scale temporal edges} capture patterns at different time scales (6/24/25+ balls)
    \item \textbf{Temporal decay edge attributes} for same\_bowler (4-over) and same\_batsman (10-over)
    \item \textbf{Causal same\_over edges} with \textbf{ball-in-over position features} via TransformerConv
    \item \textbf{Causal same\_matchup edges} prevent train-test distribution shift
    \item \textbf{Z2 symmetric striker/nonstriker states} with matching \texttt{balls\_since\_on\_strike} / \texttt{balls\_since\_as\_nonstriker}
    \item \textbf{Hierarchical player embeddings} handle cold-start (unknown players)
    \item \textbf{Enhanced FiLM modulation} conditioned on phase + chase + resource state (15 features)
    \item \textbf{Enhanced chase\_state} with 7 features (RRR, difficulty, resources)
    \item \textbf{is\_super\_over} and \textbf{is\_womens\_cricket} features for match context
    \item \textbf{balls\_since\_wicket} in dot\_pressure captures bowler momentum (R4 feedback loop)
    \item \textbf{GATv2Conv for informs edges} allows attention-weighted momentum aggregation
    \item \textbf{Innings-conditional prediction heads} separate 1st/2nd innings logic (7-dim chase injection)
    \item \textbf{Hybrid readout} combines matchup interaction with global context
    \item \textbf{Non-striker gated modulation (P1.1)} for running outcomes---gate modulates matchup based on non-striker's representation
    \item \textbf{Bowling type inference (P1.2)} from phase-specific economy patterns (pace performs better in powerplay, spin in death)
    \item \textbf{Pressure accumulation and trend (P1.3)} in dot\_pressure---exponentially weighted history and first-half vs second-half comparison
