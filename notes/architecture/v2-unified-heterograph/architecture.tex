...
Node Name & Features & What it Represents \\
\midrule
\texttt{score\_state} & 4 features & Current score, wickets, balls, innings \\
...
\texttt{phase\_state} & 5 features & Powerplay/middle/death phase, over progress, \textbf{is\_first\_ball} cold-start indicator \\
...
\texttt{nonstriker\_state} & 7 features & HOW the non-striker has been batting (includes is\_debut\_ball) \\
...
\midrule
17 numeric features & runs, is\_wicket, over, ball\_in\_over, is\_boundary, extras (wide, noball, bye, legbye), wicket types (bowled, caught, lbw, run\_out, stumped, other), \textbf{striker\_run\_out}, \textbf{nonstriker\_run\_out} \\
64-dim bowler ID & Embedding of who bowled this ball \\
64-dim batsman ID & Embedding of who faced this ball \\
\bottomrule
\end{tabular}
\caption{Ball nodes: one per historical delivery (now with run-out attribution)}
\end{table}
...
\textbf{Total per ball:} 17 + 64 + 64 = 145 features $\to$ projected to 128-dim.
...
\begin{tikzpicture}[
    ...
    \node[stage, fill=green!10] (s2) at (0, 2.5) {
        ...
        FiLM($\gamma$, $\beta$) from: phase\_state + chase\_state + wicket\_buffer (14 features)
    };
...
\begin{itemize}
    ...
    \item \textbf{N ball nodes} capture full innings history (17 features including run-out attribution)
    ...
    \item \textbf{balls\_since\_on\_strike} feature captures striker ``cold restart'' after non-striker period
    \item \textbf{Hierarchical player embeddings} handle cold-start (unknown players)
    \item \textbf{Enhanced FiLM modulation} conditioned on phase + chase + resource state (14 features)
