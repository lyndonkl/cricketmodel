...
\texttt{chase\_state} & 3 features & Target, required rate, is\_chase (2nd innings only) \\
...
\begin{tikzpicture}[
    ...
    \node[stage, fill=green!10] (s2) at (0, 2.5) {
        \textbf{Stage 2: Message Passing (with Phase Modulation)}\\[0.2cm]
        HeteroConv layers ($\times$3) with \textbf{FiLM conditioning}\\
        Each layer: conv $\to$ FiLM($\gamma$, $\beta$ from phase\_state) $\to$ residual $\to$ LayerNorm
    };
...
\textbf{same\_over (CAUSAL)} & \textbf{GATv2Conv} & \textbf{Within-over context (older$\to$newer only)} \\
...
\begin{itemize}
    ...
    \item \textbf{Causal same\_over edges} capture within-over context (bowler plans, batsman reading)
    ...
    \item \textbf{FiLM phase modulation} enables phase-conditional message passing
    \item \textbf{Innings-conditional prediction heads} separate 1st/2nd innings logic
